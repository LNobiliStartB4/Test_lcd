/* USER CODE BEGIN Header */
/**
  ******************************************************************************
  * @file           : main.h
  * @brief          : Header for main.c file.
  *                   This file contains the common defines of the application.
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  *
  ******************************************************************************
  */
/* USER CODE END Header */

/* Define to prevent recursive inclusion -------------------------------------*/
#ifndef __MAIN_H
#define __MAIN_H

#ifdef __cplusplus
extern "C" {
#endif

/* Includes ------------------------------------------------------------------*/
#include "stm32f4xx_hal.h"

/* Private includes ----------------------------------------------------------*/
/* USER CODE BEGIN Includes */

/* USER CODE END Includes */

/* Exported types ------------------------------------------------------------*/
/* USER CODE BEGIN ET */

/* USER CODE END ET */

/* Exported constants --------------------------------------------------------*/
/* USER CODE BEGIN EC */

/* USER CODE END EC */

/* Exported macro ------------------------------------------------------------*/
/* USER CODE BEGIN EM */

/* USER CODE END EM */

void HAL_TIM_MspPostInit(TIM_HandleTypeDef *htim);

/* Exported functions prototypes ---------------------------------------------*/
void Error_Handler(void);

/* USER CODE BEGIN EFP */
extern DMA_HandleTypeDef hdma_spi1_tx;
extern DMA_HandleTypeDef hdma_spi3_rx;
extern DMA_HandleTypeDef hdma_spi3_tx;

/* USER CODE END EFP */

/* Private defines -----------------------------------------------------------*/
#define B1_Pin GPIO_PIN_13
#define B1_GPIO_Port GPIOC
#define PUMP_CURRENT_Pin GPIO_PIN_0
#define PUMP_CURRENT_GPIO_Port GPIOC
#define ASSET_FLASH_CS_Pin GPIO_PIN_1
#define ASSET_FLASH_CS_GPIO_Port GPIOB
#define BUZZER_Pin GPIO_PIN_0
#define BUZZER_GPIO_Port GPIOA
#define DISP_RST_Pin GPIO_PIN_1
#define DISP_RST_GPIO_Port GPIOA
#define CAM_DISABLE_Pin GPIO_PIN_4
#define CAM_DISABLE_GPIO_Port GPIOA
#define RFID_IRQ_OUT_Pin GPIO_PIN_4
#define RFID_IRQ_OUT_GPIO_Port GPIOC
#define MPXV5100DP_Pin GPIO_PIN_5
#define MPXV5100DP_GPIO_Port GPIOC
#define DISP_DC_Pin GPIO_PIN_10
#define DISP_DC_GPIO_Port GPIOB
#define RFID_CS_Pin GPIO_PIN_12
#define RFID_CS_GPIO_Port GPIOB
#define STATUS_LED_Pin GPIO_PIN_8
#define STATUS_LED_GPIO_Port GPIOC
#define BACKLIGHT_Pin GPIO_PIN_8
#define BACKLIGHT_GPIO_Port GPIOA
#define USART1_TX_Pin GPIO_PIN_9
#define USART1_TX_GPIO_Port GPIOA
#define USART1_RX_Pin GPIO_PIN_10
#define USART1_RX_GPIO_Port GPIOA
#define TMS_Pin GPIO_PIN_13
#define TMS_GPIO_Port GPIOA
#define TCK_Pin GPIO_PIN_14
#define TCK_GPIO_Port GPIOA
#define DISP_CS_Pin GPIO_PIN_2
#define DISP_CS_GPIO_Port GPIOD
#define RFID_IRQ_IN_Pin GPIO_PIN_4
#define RFID_IRQ_IN_GPIO_Port GPIOB
#define TOUCH_RST_Pin GPIO_PIN_5
#define TOUCH_RST_GPIO_Port GPIOB
#define FRAM_CS_Pin GPIO_PIN_6
#define FRAM_CS_GPIO_Port GPIOB
#define TOUCH_IRQ_Pin GPIO_PIN_7
#define TOUCH_IRQ_GPIO_Port GPIOB
#define TOUCH_IRQ_EXTI_IRQn EXTI9_5_IRQn

/* USER CODE BEGIN Private defines */

/* USER CODE END Private defines */

#ifdef __cplusplus
}
#endif

#endif /* __MAIN_H */
