...
static bool fram_select(const fram_mb85rs256b_t *fram)
{
  (void)fram;
  return SharedSpi2_Select(SHARED_SPI2_DEVICE_FRAM, 20U);
}

static void fram_deselect(const fram_mb85rs256b_t *fram)
{
  (void)fram;
  SharedSpi2_Deselect(SHARED_SPI2_DEVICE_FRAM);
}
...
static fram_mb85rs256b_status_t fram_write_enable(const fram_mb85rs256b_t *fram)
{
  ...
  if (!fram_select(fram))
  {
    return FRAM_MB85RS256B_ERROR_HAL;
  }

  HAL_StatusTypeDef status = SharedSpi2_Transmit(&command, 1u, fram->timeout_ms);
  fram_deselect(fram);
                                                     ...
                                                     uint8_t id[FRAM_MB85RS256B_ID_LENGTH])
{
  ...
  HAL_StatusTypeDef status = SharedSpi2_TransmitReceive(tx,
                                                       raw,
                                              ...
                                              size_t length)
{
  ...
  HAL_StatusTypeDef status = SharedSpi2_Transmit(header, sizeof(header), fram->timeout_ms);
  if (status == HAL_OK)
  {
    status = SharedSpi2_Receive(data, (uint16_t)length, fram->timeout_ms);
  }
  fram_deselect(fram);
                                               ...
                                               size_t length)
{
  ...
  if (status == HAL_OK)
  {
    status = SharedSpi2_Transmit(data, (uint16_t)length, fram->timeout_ms);
  }
  fram_deselect(fram);
